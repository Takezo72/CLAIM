...
{
    [Serializable]
    public class ReviewInsured : IValidatableObject
    {
        ...
        public string NouvelleAdresse { get; set; }
        ...
        private void AjouterValidationTelSecondairePosteDemandeur(List<ValidationResult> result)
        {
            if (DeclarantAssure == "N")
            {
                if (!(string.IsNullOrEmpty(TelSecondairePosteDemandeur)) && !Helpers.ValidationHelper.IsIntegerValid(TelPrincipalPosteDemandeur))
                {
